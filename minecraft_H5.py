...
class MinecraftEnv(gym.Env):
    ...
    def _HERreplay(self, state, newState, action_input):
        ...
        if action in range(1,5):     # Move
            ...
            if self.world.getBlock(source,worldState,1) in [-1, -2] and self.world.getBlock(dest,worldState,1) in [0, -2]\
                ...
                if self.world.getBlock(dest,worldState,1) == -2: # Blocks can be destroyed by placing them in a source. However, we
                    ...
                    if self.FULL_HELP:
                        if np.sum(np.clip(newWorldState, -1, 0)) > np.sum(np.clip(newPlan, -1, 0)):
                            ...
                            reward += PLACE_REWARD
                elif self.world.getBlock(source,worldState,1) == -2: # Make a block appear above the agent
                    if self.FULL_HELP:
                        if np.sum(np.clip(newWorldState, -1, 0)) < np.sum(np.clip(newPlan, -1, 0)):
                            reward -= PLACE_REWARD
                        elif np.sum(np.clip(newWorldState, -1, 0)) > np.sum(np.clip(newPlan, -1, 0)):
                    ...
                    if action > 8 and newPlan[dest[0], dest[1], dest[2]] == -1: # Place correct block
                        reward += PLACE_REWARD * (dest[1]+1)**2
                    elif action < 9 and newPlan[source[0], source[1], source[2]] == -1: # Removing correct block
                        reward -= PLACE_REWARD * (source[1]+1)**2
                    elif action > 8 and newPlan[dest[0], dest[1], dest[2]] == 0 and complete: # Place incorrect block
                        reward -= PLACE_REWARD
                    elif action < 9 and newPlan[source[0], source[1], source[2]] == 0 and complete: # Remove incorrect block
                        reward += PLACE_REWARD

        # Done?
        done, is_built = self.world.done(newPlan,newWorldState,1)
        has_block = self.world.getBlock((new_agent_pos + np.array([0,1,0])),newWorldState,1) == -1
    ...
    def _render(self, agent_id=1, mode='human', close=False):
        ...
        window.save_pics(data, './demo1/step{}'.format(self.step_num))
