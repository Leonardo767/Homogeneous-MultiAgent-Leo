...
PLAN_MAPS = [
    ...
    # 6 Center tower
    [[4, 0, 4], [4, 1, 4], [4, 2, 4], [4, 3, 4], [4, 4, 4], [4, 5, 4], [4, 6, 4]],
    # 7 Test tower
...
class Grid3DState(object):
    ...
    def getObservation(self, coord, ob_range):
        ...
        note: observation.shape is (
            2*ob_range[0]+1, 2*ob_range[1]+1, 2*ob_range[2]+1)
        '''
...
class MinecraftEnv(gym.Env):
    ...
    # Initialize env
    def __init__(self, num_agents=1, observation_range=1, observation_mode='id_visible', world0=None, FULL_HELP=False, MAP_ID=1, CURRICULUM=False, current_screenshots=[]):
        """
        Args:
        ...
        # Curriculum: track the difficulty level (height of the target building)
        if CURRICULUM:
            self.difficultyLevel = 1
            self.maxDifficulty = len(PLAN_MAPS[self.map_id])
        else:
            self.difficultyLevel = -1
    ...
    def _setObjective(self):
        ...
        if self.RANDOMIZED_PLANS:
            ...
            # not in a curriculum case, set the goal to be the whole building
            if (self.difficultyLevel == -1) or (self.difficultyLevel > len(PLAN_MAPS[self.map_id])):
                for pos in PLAN_MAPS[self.map_id]:
                    plan_map[pos[0], pos[1], pos[2]] = -1
            else:
                for pos in PLAN_MAPS[self.map_id][:self.difficultyLevel]:
                    plan_map[pos[0], pos[1], pos[2]] = -1
    ...
    def _reset(self, agent_id, empty=False, full=False):
        self.finished = False
        self.mutex.acquire()
        # increase the plan height (used in setObjective())
        if not self.fresh:
            # Check everything is alright
            assert self.state_init.shape == self.state_obj.shape, '\'state_init\' and \'state_obj\' dimensions do not match'
